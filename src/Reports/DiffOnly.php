<?php
/**
 * Full diff report for PHP_CodeSniffer.
 */
...
class DiffOnly implements Report
{
    ...
    {
        if ($report['diffErrors'] === 0 && $report['diffWarnings'] === 0) {
            // Nothing to print.
            return false;
        }

        // The length of the word ERROR or WARNING; used for padding.
        if ($report['diffWarnings'] > 0) {
            $typeLength = 7;
        } else {
            $typeLength = 5;
        }
        ...
        echo "\033[1m".'FOUND '.$report['diffErrors'].' ERROR';
        if ($report['diffErrors'] !== 1) {
            echo 'S';
        }

        if ($report['diffWarnings'] > 0) {
            echo ' AND '.$report['diffWarnings'].' WARNING';
            if ($report['diffWarnings'] !== 1) {
                echo 'S';
            }
        }

        echo ' AFFECTING '.$report['diffLines'].' LINE';
        if ($report['diffLines'] !== 1) {
            echo 'S';
        }
        ...
        echo str_repeat('-', $width).PHP_EOL;
        if ($report['diffFixable'] > 0) {
            echo "\033[1m".'PHPCBF CAN FIX THE '.$report['diffFixable'].' MARKED SNIFF VIOLATIONS AUTOMATICALLY'."\033[0m".PHP_EOL;
            echo str_repeat('-', $width).PHP_EOL;
        }
